...
function AutoHeightImage(props) {
  const {onHeightChange, source, width, style, maxHeight, onError, ...rest} = props;
  const [height, setHeight] = useState(
    getImageSizeFitWidthFromCache(source, width, maxHeight).height || DEFAULT_HEIGHT,
  );
  ...
  useEffect(
    function () {
      (async function () {
        try {
          const {height: newHeight} = await getImageSizeFitWidth(source, width, maxHeight);
  ...
  return (
    <ImageComponent source={source} style={[imageStyles, style]} onError={onError} {...rest} />
  );
}
